...
class Cart_mapper extends MY_Model
{
    ...
    public function  __construct() {
        ...
        $this->_template['checkout_1'] = 'cart/checkout_1';
        $this->_template['checkout_2'] = 'cart/checkout_2';
        $this->_template['checkout_3'] = 'cart/checkout_3';
    ...
    public function get_page_content($page_id = 0)
    {
        ...
        {
            if ( $_REQUEST['type'] == 'order_1' )
            {
                return $this->load->site_view($this->_template['checkout_1'], $template_data, TRUE);
            }
            if ( $_POST['type'] == 'order' )
            {
            ...
            if ( $_POST['type'] == 'save_3' )
            {
                $this->form_validation->set_rules('name', 'Имя', 'required');
                $this->form_validation->set_rules('fename', 'Фамилия', 'required');
                ...
                $this->form_validation->set_rules('imail', 'Почтовый индекс', 'required');
                $this->form_validation->set_rules('city', 'Город', 'required');
                $this->form_validation->set_rules('street', 'Улица', 'required');
                $this->form_validation->set_rules('home', 'Дом', 'required');
                ...
                if ( $this->form_validation->run() )
                {
                    if ( $total )
                    {
                        $adress = 'Страна: '.$this->input->post('country').' Почтовый индекс: '.$this->input->post('imail').' Регион: '.$this->input->post('region').' Город: '.$this->input->post('city').' Улица: '.$this->input->post('street').' Дом: '.$this->input->post('home').' Квартира: '.$this->input->post('room').'';
                        $now = date("Y-m-d H:i:s", time());
                        // save order
                        $order_data = array();
                        $order_data['name']            = $this->input->post('name');
                        $order_data['fename']          = $this->input->post('fename');
                        $order_data['phone']           = $this->input->post('phone');
                        $order_data['address']         = $adress;
                        $order_data['email']           = $this->input->post('email');
                        $order_data['comments']        = $this->input->post('delivery');
                        $order_data['payment_method']  = $this->input->post('payments');
    ...
    private function _send_mails ( $user_email, $order_id, $total_price )
    {
        ...
        $userinfo .= '<p>Фамилия - '.$db_result['fename'].'</p>';
        ...
        $userinfo .= '<p>Доставка - '.$this->_com_method($db_result['comments']).'</p>';
        $userinfo .= '</div></br>';
    ...
    public function _payment_method ( $index = 0 )
    {
        $result = array();
        $result[0] = 'unknown';
        $result[1] = 'Наличными при получении';
        $result[2] = 'Другим способом';
    ...
    public function _com_method ( $index = 0 )
    {
        $result = array();
        $result[0] = 'unknown';
        $result[1] = 'Куръерская доставка';
        $result[2] = 'Самовывоз';
        if ( isset($result[$index]) )
        {
            return $result[$index];
