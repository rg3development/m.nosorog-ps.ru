...
<div id="content">
    <form action="<?= base_url("admin/banner/edit_image/{$data['id']}/{$parent_id}"); ?>" method="post" enctype="multipart/form-data">
        <table class="admin_module_form">
            ...
            <tr>
                <td class="admin_module_form_title">ссылка</td>
                <td><input type="text" name="link" value='<?=$data['link'];?>' /></td>
            </tr>
            <tr>
                <td class="admin_module_form_title">открывать в новой вкладке</td>
                <td><input <?=$data['link_new_window'] ? "checked='checked'" : ""?>type="checkbox" name="link_new_window[]" value='newwindow' /></td>
            </tr>
            <tr>
            ...
            <tr>
                <td class="admin_module_form_title">изображение</td>
                <td><input type="file" name="image" size="255" />&nbsp;<strong><?= BANNER_MAIN_IMAGE_TOOLTIP; ?></strong></td>
            </tr>
            <? if (!empty($data['image']) && $data['image'] != '_thumb') : ?>
                <tr>
                    <td></td>
                    <td><img src="<?=$data['path'].'/'.$data['image'];?>"/></td>
                </tr>
            <? endif; ?>
            <tr>
                <td class="admin_module_form_title">текст</td>
