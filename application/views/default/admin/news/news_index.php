            ...
            <div class="admin_module_list">
                ...
                <?php foreach ($list as $key => $item) :?>
                    <li id="text<?=$item->id;?>" onmouseover="$(this).children().children().css('color', '#0099cc')" onmouseout="$(this).children().children().css('color', '#666666')">
                        <a href="<?= base_url("admin/news/edit_news/{$item->id}/{$parent_id}"); ?>" title="редактировать">
                            <em><?=$item->title;?></em>
                        </a>
                        <a href="<?= base_url("admin/news/delete_news/{$item->id}/{$parent_id}"); ?>" class="admin_form_action_page" onclick="if (confirm('Вы уверены?')) return true; else return false;" title="удалить">
                            <img src="<?= base_url('www_admin/img/icon_delete_1.5.png'); ?>" />
                        </a>
                        <a href="<?= base_url("admin/news/edit_news/{$item->id}/{$parent_id}"); ?>" class="admin_form_action_page" title="редактировать">
                            <img src="<?= base_url('www_admin/img/icon_edit_1.5.png'); ?>" />
                        </a>
