                    ...
                    <?php if (!empty($list)) :?>
                        <?php foreach ($list as $news) :?>
                            <li id="text<?=$news->id;?>" onmouseover="$(this).children().children().css('color', '#0099cc')" onmouseout="$(this).children().children().css('color', '#666666')">
                                ...
                                <a href="<?= base_url("admin/news/delete/{$news->id}/{$parent_id}"); ?>" class="admin_form_action_page" onclick="if (confirm('Вы уверены?')) return true; else return false;" title="удалить">
                                    <img title="удалить" alt="удалить" src="<?=base_url('www_admin/img/icon_delete_1.5.png');?>" />
                                </a>
                                <a href="<?= base_url("admin/news/edit/{$news->id}/{$parent_id}"); ?>" class="admin_form_action_page" title="редактировать">
                                    <img title="редактировать" alt="редактировать" src="<?=base_url('www_admin/img/icon_edit_1.5.png');?>" />
                                </a>
                                <a href="<?= base_url("admin/news/copy/{$news->id}/{$parent_id}"); ?>" class="admin_form_action_page" title="копировать">
