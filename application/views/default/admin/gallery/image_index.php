...
<div id="content">
    <div class="photo_module">
        <div class="admin_module_form right_buttons">
            <a href="<?= base_url("admin/gallery/add_image/{$parent_id}"); ?>">
                <button class="g-button">создать</button>
            </a>
        </div>
                ...
                <? if ( ! empty($list) ) : ?>
                    <? foreach ( $list as $key => $item ) : ?>
                        <li id="text<?=$item->id;?>" onmouseover="$(this).children().children().css('color', '#0099cc')" onmouseout="$(this).children().children().css('color', '#666666')">
                            ...
                            <a href="<?= base_url("admin/gallery/delete_image/{$item->id}/{$parent_id}"); ?>" class="admin_form_action_page" onclick="if (confirm('Вы уверены?')) return true; else return false;" title="удалить">
                                <img title="удалить" alt="удалить" src="<?= base_url('/www_admin/img/icon_delete_1.5.png'); ?>"/>
                            </a>
                            <a href="<?= base_url("admin/gallery/edit_image/{$item->id}/{$parent_id}"); ?>" class="admin_form_action_page" title="редактировать">
